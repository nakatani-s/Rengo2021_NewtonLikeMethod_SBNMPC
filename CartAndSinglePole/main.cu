...
int main(int argc, char **argv)
{
    ...
    CHECK( cudaMalloc(&Gradient, sizeof(double) * HORIZON) );
    ...
    hostData = (double *)malloc(sizeof(double) * HORIZON);
    hostTempData = (double *)malloc(sizeof(double) * HORIZON);
    CHECK(cudaMalloc(&deviceData, sizeof(double) * HORIZON));
    cudaMalloc(&deviceTempData, sizeof(double) * HORIZON);
    ...
    CHECK(cudaMalloc((void**)&hQR_tau, sizeof(double) * HORIZON));
#endif
    ...
    for(int t = 0; t < SIM_TIME; t++)
    {
        ...
        if(t == 0)
        {
            ...
            for(int iter = 0; iter < ITERATIONS; iter++)
            {
                ...
                CHECK_CUSOLVER( cusolverDnDpotrf_bufferSize(cusolverH, uplo, HORIZON, Hessian, HORIZON, &w_si_hessian), "Failed to get bufferSize of computing the inverse of Hessian");
                ...
                CHECK_CUSOLVER( cusolverDnDpotrf(cusolverH, uplo, HORIZON, Hessian, HORIZON, w_sp_hessian, w_si_hessian, devInfo), "Failed to inverse operation");
                ...
                CHECK_CUSOLVER( cusolverDnDpotrs(cusolverH, uplo, HORIZON, HORIZON, Hessian, HORIZON, invHessian, HORIZON, devInfo), "Failed to get inverse of Hessian");
                // 逆行列を-1倍する操作
                ...
                CHECK_CUBLAS(cublasDgemv(handle_cublas, CUBLAS_OP_N, HORIZON, HORIZON, &alpha, Hessian, HORIZON, Gradient, 1, &beta, deviceTempData, 1), "Failed to get result by proposed method");
#else
                if(t==1){
                    ...
                    CHECK_CUSOLVER( cusolverDnDgeqrf_bufferSize(cusolverH, HORIZON, HORIZON, Hessian, HORIZON, &geqrf_work_size), "Failed to get buffersize for QR decom [1]" );
                ...
                CHECK_CUSOLVER( cusolverDnDgeqrf(cusolverH, HORIZON, HORIZON, Hessian, HORIZON, hQR_tau, w_sp_hessian, w_si_hessian, devInfo),"Failed to compute QR factorization" );
                ...
                CHECK_CUSOLVER( cusolverDnDormqr(cusolverH, side, trans, HORIZON, nrhs, HORIZON, Hessian, HORIZON, hQR_tau, Gradient, HORIZON, w_sp_hessian, w_si_hessian, devInfo), "Failed to compute Q^T*B" );
                CHECK(cudaDeviceSynchronize());
                ...
                CHECK_CUBLAS( cublasDtrsm(handle_cublas, side, uplo_QR, trans_N, cub_diag, HORIZON, nrhs, &m_alpha, Hessian, HORIZON, Gradient, HORIZON), "Failed to compute X = R^-1Q^T*B" );
                CHECK(cudaDeviceSynchronize());

                NewtonLikeMethodCopyVector<<<HORIZON, 1>>>(deviceTempData, Gradient);
                CHECK(cudaDeviceSynchronize());
#endif
                CHECK( cudaMemcpy(hostTempData, deviceTempData, sizeof(double) * HORIZON, cudaMemcpyDeviceToHost) );
                CHECK(cudaDeviceSynchronize());
        ...
        printf("MCMPC optimum condition := %lf  Proposed optimum condition := %lf\n", optimumConditions[1], optimumCondition_p[1]);
