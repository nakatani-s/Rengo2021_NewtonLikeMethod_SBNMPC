...
InputSequences::InputSequences(const InputSequences& x){ }

InputSequences& InputSequences::operator=(const int size)
{
    _realloc_data(size);
...
__host__ __device__ double& InputSequences::operator[](int pos)
{
    return data[pos];
}

void InputSequences::_realloc_data(int len)
{
    cudaFree(data);
    length = len;
