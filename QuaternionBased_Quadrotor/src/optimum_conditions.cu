...
void calc_Cost_QuaternionBased_Quadrotor(double *cost, double *U, SystemControlVariable *SCV)
{
    double costValue[2] = { };
    double stageCost = 0.0;
    double logBarrier = 0.0;
    ...
    for(int hStep = 0;  hStep < HORIZON; hStep++)
    {
        uIndex = hStep * DIM_OF_INPUT;
        // dynamics_ddot_Quadrotor(dstate, U[uIndex], U[uIndex + 1], U[uIndex + 2], U[uIndex + 3], state, SCV);
        dynamics_QuaternionBased_Quadrotor(dstate, U[uIndex], U[uIndex + 1], U[uIndex + 2], U[uIndex + 3], state, SCV);
        for(int stIndex = 0; stIndex < DIM_OF_STATES; stIndex++)
        {
            state[stIndex] = state[stIndex] + ( dstate[stIndex] * d_time );
        }
        ...
        stageCost = SCV->weightMatrix[0] * (state[0] - SCV->params[1]) * (state[0] - SCV->params[1])
                    ...
                    + SCV->weightMatrix[5] * state[5] * state[5] + SCV->weightMatrix[6] * state[6] * state[6]
                    + SCV->weightMatrix[7] * state[7] * state[7] + SCV->weightMatrix[8] * state[8] * state[8]
                    + SCV->weightMatrix[9] * state[10] * state[10] + SCV->weightMatrix[10] * state[11] * state[11]
                    + SCV->weightMatrix[11] * state[12] * state[12]
                    + SCV->weightMatrix[12] * (U[uIndex] - SCV->params[0]) * (U[uIndex] - SCV->params[0]) + SCV->weightMatrix[13] * U[uIndex+1] * U[uIndex+1]
                    + SCV->weightMatrix[14] * U[uIndex+2] * U[uIndex+2] + SCV->weightMatrix[15] * U[uIndex+3] * U[uIndex+3];
